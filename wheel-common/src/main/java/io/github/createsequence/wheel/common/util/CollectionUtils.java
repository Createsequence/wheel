...
public class CollectionUtils {
    ...
    /**
     * <p>键集合与值集合合并为{@link Map}，当键重复时，总是使用新值覆盖旧值。
     * <b>注意:</b>集合的长度取决于键集合的长度，值集合长度超过键集合长度的部分会被舍弃，
     * 而键集合长度超过值集合长度的部分将会把值全部设置为{@code null}.
     *
     * @param keys 键集合
     * @param values 值集合
     * @return 集合
    ...
    public static <K, V> Map<K, V> zip(Collection<K> keys, Collection<V> values) {
        return zip(keys, values, (oldVal, newVal) -> newVal);
    }

    /**
     * <p>键集合与值集合合并为{@link Map}。
     * <b>注意:</b>集合的长度取决于键集合的长度，值集合长度超过键集合长度的部分会被舍弃，
     * 而键集合长度超过值集合长度的部分将会把值全部设置为{@code null}.
     *
     * @param keys 键集合
     * @param values 值集合
     * @param remapping 当键集合重复时，应取哪个值作为最终存储值
     * @return 集合
     */
    public static <K, V> Map<K, V> zip(
        Collection<K> keys, Collection<V> values, BinaryOperator<V> remapping) {
        ...
        while (keysIterator.hasNext()) {
            ...
            V newVal = valuesIterator.hasNext() ? valuesIterator.next() : null;
            results.compute(key, (k, v) -> Optional.ofNullable(v)
                .map(oldVal -> remapping.apply(oldVal, newVal))
                .orElse(newVal)
            );
        }
        return results;
